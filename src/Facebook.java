...
public class Facebook {
	...
	public static void main(String[] args) {
				...
				if (aPost.getMessage() != null && aPost.getMessage().contains("papacocas")) {
					...
					for (List<Comment> postcomments : allComments) {
						for (Comment comment : postcomments) {
							System.out.println("nb like : " + comment.getLikeCount()); // nb
																						// aime
		 ...
		 * 
		 * Connection<User> publicSearch = fbClient.fetchConnection("search",
		 * User.class, Parameter.with("q", "Dylan"), Parameter.with("type",
		 * "user")); for(int i=0;i<publicSearch.getData().size();i++){
		 * System.out.println("Dylan de ID " +
		 * publicSearch.getData().get(i).getId() + " se nomme " +
		 * publicSearch.getData().get(i).getName()+" : " +
		 ...
		 * "page")); for(int i=0;i<publicSearch.getData().size();i++){
		...
		// Mes Likes
		/*
		 * int counter = 0; Connection<Page> result =
		 * fbClient.fetchConnection("me/likes", Page.class);
		 * 
		 * System.out.println("\n\n\n"+result);
		 * 
		 * for(List<Page> feedPage : result){ for(Page page2 : feedPage){
		 * System.out.println(page2.getName());
		 * System.out.println("fb.com/"+page2.getId()); counter++;
		 * 
		...
		// Mes Amis
		/*
		 * Connection<User> myFriends = fbClient.fetchConnection("me/friends",
		 * User.class); System.out.println("Count of my friends :" +
		 * myFriends.getData().size());
		 */

		// Récupérer le contenu HTML de ma page d'ami Facebook
		/*
		 * HttpURLConnection conn; try { conn = (HttpURLConnection) new
		 * URL("https://www.facebook.com/me/friends").openConnection();
		 * conn.connect(); BufferedInputStream bis = new
		 * BufferedInputStream(conn.getInputStream()); byte[] bytes = new
		 * byte[1024]; int tmp ; while( (tmp = bis.read(bytes) ) != -1 ) {
		 * String chaine = new String(bytes,0,tmp); System.out.print(chaine); }
		 * 
		 * conn.disconnect(); } catch (MalformedURLException e) { // TODO
